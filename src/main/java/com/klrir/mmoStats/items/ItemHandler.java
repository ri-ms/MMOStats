...
import com.klrir.mmoStats.items.enchantments.CustomEnchantment;
import com.klrir.mmoStats.utils.Assert;
import com.klrir.mmoStats.utils.ReflectionUtils;
import com.klrir.mmoStats.utils.inventories.items.Items;
...
public class ItemHandler {
    ...
    public static <T, Z> boolean hasPDC(String str, ItemStack item, PersistentDataType<T, Z> type) {
        if (item != null && item.getItemMeta() != null) {
            return item.getItemMeta().getPersistentDataContainer().has(new NamespacedKey(MMOStats.getInstance(), str), type);
        }
        return false;
    }

    public static <T, Z> Z getPDC(String str, ItemStack item, PersistentDataType<T, Z> type) throws NullPointerException {
        return item.getItemMeta().getPersistentDataContainer().get(new NamespacedKey(MMOStats.getInstance(), str), type);
    }
    ...
    public static <T, Z> Z getOrDefaultPDC(String str, ItemStack item, PersistentDataType<T, Z> type, Z def) throws NullPointerException {
        return item.getItemMeta().getPersistentDataContainer().getOrDefault(new NamespacedKey(MMOStats.getInstance(), str), type, def);
    }

    public static <T, Z> void setPDC(String str, ItemStack item, PersistentDataType<T, Z> type, Z val) {
        ItemMeta meta = item.getItemMeta();
        meta.getPersistentDataContainer().set(new NamespacedKey(MMOStats.getInstance(), str), type, val);
        item.setItemMeta(meta);
    }

    public static void removePDC(String str, ItemStack item) {
        ItemMeta meta = item.getItemMeta();
        meta.getPersistentDataContainer().remove(new NamespacedKey(MMOStats.getInstance(), str));
        item.setItemMeta(meta);
    }
    ...
    public static void registerAll(Object obj) {
        ...
        for (Method method : obj.getClass().getMethods())
            if (method.getAnnotation(com.klrir.mmoStats.items.ItemRegistration.ItemHandler.class) != null) {
